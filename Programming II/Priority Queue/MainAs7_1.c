...
//Programming 2
//04/07/17
//Output: Menu consisting of add, fetch, print, and quit./Prompt to add to queue and priority/Item fetched and integer/
//A list of the integers in the queue./Various error messages.

#include "BoolAs7_1.h"												//Include header files used in main
#include "ProtoAs7_1.h"
#include <stdio.h>
#include "DefsAs7_1.c"											//Do not use in VS
...
int main()
{
	...
	while (quit == FALSE)											//while loop on quit option
	{
		...
		case 1:														//if menu option var is 1
		{
			...
			{
				int item, pri;										//create a var for entering to queue and prioritizing
				printf("Enter an Integer:");						//prompt to enter an integer for the var
				scanf("%d", &item);									//assign input to var for queue
				printf("give it a priority:");						//prompt to enter an interger for prioritization
				scanf("%d", &pri);									//assign input to var for priority
				add(&myqueue, item, pri);							//call add and send it queue, var to add, and priority number
				printf("\n");
			}
